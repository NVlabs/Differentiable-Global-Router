...
cd $cugr2/run/
./route -lef $benchmark_path/$data/$data.input.lef -def $benchmark_path/$data/$data.input.def -output $benchmark_path/$data/$data.CUGR2_NewSort.guide -sort 0 > $cugr2/GR_log/$data\_CUGR2_NewSort.log

...
def train_model(config):
    ...
    subprocess.run(["python", "main_stochastic.py", "--data_path", os.path.join(cugr2, "run", f"{data}.pt"), "--optimizer", str(config['optimizer']), "--output_name", str(task_id)])
...
results_df.to_csv(f"./ray_results/{data}_opt.csv", mode='a', header=True)
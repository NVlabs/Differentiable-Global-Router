...
def train_model(config):
    ...
    subprocess.run(["python", "main_stochastic.py", "--data_path", os.path.join(cugr2, "run", f"{data}.pt"), "--lr", str(config['learning_rate']), "--output_name", str(task_id)])
...
results_df.to_csv(f"./ray_results/{data}_lr.csv", mode='a', header=True)
...
def train_model(config):
    ...
    subprocess.run(["python", "main_stochastic.py", "--data_path", os.path.join(cugr2, "run", f"{data}.pt"), "--scheduler", str(config['scheduler']), "--output_name", str(task_id)])
...
config = {
    # "learning_rate": tune.loguniform(1e-5, 10),
...
results_df.to_csv(f"./ray_results/{data}_scheduler.csv", mode='a', header=True)